...
describe("governance – getDelegationInfo", () => {
  it("returns delegation data for known address GABC123", async () => {
    ...
    expect(info.delegatedTo).toBe(
      "GDEF456EXAMPLE789ABC012GHI345JKL678MNO901PQR234STU567VWX890YZ"
    );
...
describe("governance – lookupToken", () => {
  ...
  it("rejects address that does not start with G", async () => {
    await expect(lookupToken("CABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCD")).rejects.toThrow(
      "Invalid Stellar address"
    );
  });
  ...
  it("returns Unknown Token for valid G-address not in known set", async () => {
    ...
    const promise = lookupToken(
      "GBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBVN"
    );
    vi.runAllTimers();
