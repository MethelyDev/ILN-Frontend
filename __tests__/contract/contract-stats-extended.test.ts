...
function makeInvoice(overrides: Partial<Invoice> = {}): Invoice {
  return {
    id: 1n, status: "Pending", freelancer: "G1", payer: "G2",
    amount: 100_000_000n, due_date: BigInt(Math.floor(Date.now() / 1000) + 86400 * 30),
    discount_rate: 250, token: USDC, ...overrides,
  };
}
...
describe("contract-stats – XLM token path", () => {
  ...
  it("calculates volume for XLM invoices", async () => {
    vi.mocked(getAllInvoices).mockResolvedValue([
      makeInvoice({ id: 1n, status: "Funded", token: XLM, amount: 10_000_000n, funded_at: recentTs }),
    ]);
  ...
  it("handles undefined token (defaults to USDC)", async () => {
    vi.mocked(getAllInvoices).mockResolvedValue([
      makeInvoice({ id: 1n, status: "Funded", token: undefined, amount: 100_000_000n, funded_at: recentTs }),
    ]);
    const stats = await get_contract_stats();
    expect(stats.total_funded).toBe(1);
  ...
  it("handles unknown token falling back to USDC symbol", async () => {
    vi.mocked(getAllInvoices).mockResolvedValue([
      makeInvoice({ id: 1n, status: "Funded", token: "CUNKNOWNTOKEN", amount: 100_000_000n, funded_at: recentTs }),
    ]);
    const stats = await get_contract_stats();
    expect(stats.total_funded).toBe(1);
  });
...
describe("buildHistoricalVolume – XLM and unknown token paths", () => {
  it("accumulates XLM volume into xlm bucket", () => {
    const invoices = [makeInvoice({ status: "Funded", token: XLM, amount: 10_000_000n, funded_at: recentTs })];
  ...
  it("accumulates EURC volume into eurc bucket", () => {
    ...
    const invoices = [makeInvoice({ status: "Paid", token: EURC, amount: 10_000_000n, funded_at: recentTs })];
    const buckets = buildHistoricalVolume(invoices, 7);
  ...
  it("accumulates unknown token into usdc bucket as fallback", () => {
    const invoices = [makeInvoice({ status: "Funded", token: "CUNKNOWNTOKEN", amount: 10_000_000n, funded_at: recentTs })];
    const buckets = buildHistoricalVolume(invoices, 7);
