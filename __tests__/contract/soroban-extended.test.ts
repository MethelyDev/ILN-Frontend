...
const { mockServer, mockTx, mockAssembledTx } = vi.hoisted(() => {
  ...
  const mockServer: any = {
    ...
    getAccount: vi.fn(() => Promise.resolve({ accountId: () => "GAAA", incrementSequenceNumber: vi.fn(), sequenceNumber: () => "100" })),
...
vi.mock("@stellar/stellar-sdk", async (importOriginal) => {
  ...
  return {
    ...
    Contract: vi.fn(function(this: any) { this.call = vi.fn(() => ({})); }),
...
import {
  getAllInvoices, getWalletRoles, getNativeXlmBalance,
  getUsdcBalance, getTokenAllowance, approveToken, getUsdcAllowance,
  appealDefault, disputeInvoice, updateLPWhitelist, claimDefault,
  updateInvoice, getReferralStats, submitInvoiceTransaction,
  buildApproveTokenTransaction, buildApproveUsdcTransaction,
  submitSignedTransaction, convertInvoiceToken,
  getInsurancePoolInfo, getLPInsuranceStatus, depositPremium, claimInsurance,
  getReputation, getReputationEvents, getPayerScoresBatch, getTopPayers,
  getInvoice, getInvoiceCount, submitInvoicesBatch,
} from "@/utils/soroban";
...
function setupSuccess(retval: any = {}) {
  ...
  mockServer.getAccount.mockResolvedValue({ accountId: () => ADDR, incrementSequenceNumber: vi.fn(), sequenceNumber: () => "100" });
...
describe("soroban – getAllInvoices", () => {
  ...
  it("returns invoices until consecutive failure", async () => {
    const inv = { id: 1n, status: { Pending: null }, freelancer: ADDR, payer: ADDR, amount: 100n, due_date: 1n, discount_rate: 100 };
...
describe("soroban – getWalletRoles", () => {
  it("identifies freelancer role", async () => {
    const inv = { id: 1n, status: { Pending: null }, freelancer: ADDR, payer: "OTHER", amount: 100n, due_date: 1n, discount_rate: 100 };
    (scValToNative as any).mockReturnValueOnce(inv);
    (rpc.Api.isSimulationSuccess as any).mockReturnValueOnce(true).mockReturnValueOnce(false);
    mockServer.simulateTransaction
      .mockResolvedValueOnce({ result: { retval: {} } })
...
describe("soroban – updateLPWhitelist", () => {
  it("throws unsupported error", async () => {
    await expect(updateLPWhitelist({ invoiceId: 1n, whitelist: [] })).rejects.toThrow("not supported");
  });
});
...
describe("soroban – updateInvoice", () => {
  ...
  it("builds an update_invoice transaction", async () => {
    const r = await updateInvoice({ freelancer: ADDR, invoiceId: 1n, amount: 200n, dueDate: 1893456000, discountRate: 300 });
    expect(r.tx).toBeDefined();
  });
  it("throws on simulation failure", async () => {
    (rpc.Api.isSimulationSuccess as any).mockReturnValue(false);
    mockServer.simulateTransaction.mockResolvedValue({ error: "fail" });
    await expect(updateInvoice({ freelancer: ADDR, invoiceId: 1n, amount: 200n, dueDate: 1893456000, discountRate: 300 })).rejects.toThrow();
  });
});
...
describe("soroban – submitInvoiceTransaction", () => {
  ...
  it("returns invoiceId and txHash on success", async () => {
    const r = await submitInvoiceTransaction({
      freelancer: ADDR, payer: ADDR, amount: 100n, dueDate: 1893456000,
      discountRate: 250, signTx: async () => "signedXDR",
    });
  ...
  it("throws on simulation failure", async () => {
    ...
    await expect(submitInvoiceTransaction({
      freelancer: ADDR, payer: ADDR, amount: 100n, dueDate: 1893456000,
      discountRate: 250, signTx: async () => "signedXDR",
  ...
  it("includes referral code when provided", async () => {
    const r = await submitInvoiceTransaction({
      freelancer: ADDR, payer: ADDR, amount: 100n, dueDate: 1893456000,
      discountRate: 250, signTx: async () => "signedXDR", referralCode: "REF",
    });
...
describe("soroban – submitSignedTransaction", () => {
  ...
  it("throws on invalid send response", async () => {
    mockServer.sendTransaction.mockResolvedValue({});
    await expect(submitSignedTransaction({ tx: mockTx, signTx: async () => "x" })).rejects.toThrow("invalid");
  });
  it("throws on rejected status", async () => {
    mockServer.sendTransaction.mockResolvedValue({ hash: "h", status: "ERROR" });
    await expect(submitSignedTransaction({ tx: mockTx, signTx: async () => "x" })).rejects.toThrow("failed with status");
  });
...
describe("soroban – getReputationEvents", () => {
  ...
  it("filters events with invalid timestamps", async () => {
    setupSuccess([{ type: "paid", timestamp: 0 }, { type: "submitted", timestamp: 500 }]);
    (scValToNative as any).mockReturnValue([{ type: "paid", timestamp: 0 }, { type: "submitted", timestamp: 500 }]);
    const evts = await getReputationEvents(ADDR);
    expect(evts).toHaveLength(1);
  });
...
describe("soroban – getTopPayers", () => {
  it("returns array of top payers on success", async () => {
    setupSuccess([{ address: ADDR, score: 90, invoices_paid: 10, invoices_defaulted: 0, total_volume: 1000n }]);
    (scValToNative as any).mockReturnValue([{ address: ADDR, score: 90, invoices_paid: 10, invoices_defaulted: 0, total_volume: 1000n }]);
...
describe("soroban – getWalletRoles extended", () => {
  it("identifies payer role", async () => {
    setupSuccess();
    const inv = { id: 1n, status: { Pending: null }, freelancer: "OTHER", payer: ADDR, amount: 100n, due_date: 1n, discount_rate: 100 };
    (scValToNative as any).mockReturnValueOnce(inv);
    (rpc.Api.isSimulationSuccess as any).mockReturnValueOnce(true).mockReturnValueOnce(false);
    mockServer.simulateTransaction
      .mockResolvedValueOnce({ result: { retval: {} } })
  ...
  it("identifies lp role from funder field", async () => {
    setupSuccess();
    const inv = { id: 1n, status: { Funded: null }, freelancer: "OTHER", payer: "OTHER2", funder: ADDR, amount: 100n, due_date: 1n, discount_rate: 100 };
    (scValToNative as any).mockReturnValueOnce(inv);
    (rpc.Api.isSimulationSuccess as any).mockReturnValueOnce(true).mockReturnValueOnce(false);
    mockServer.simulateTransaction
      .mockResolvedValueOnce({ result: { retval: {} } })
...
describe("soroban – getInvoice parseStatus branches", () => {
  ...
  it("parses string status directly", async () => {
    (scValToNative as any).mockReturnValue({
      id: 1n, freelancer: ADDR, payer: ADDR, amount: 100n,
      due_date: 1n, discount_rate: 100, status: "Pending",
      funder: undefined, funded_at: undefined, token: USDC,
    });
